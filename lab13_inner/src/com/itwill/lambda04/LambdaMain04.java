...
//        employees.forEach((x) -> System.out.println(x));
		...
		double mean = employees.stream().filter((e) -> e.getDept().contains("개발"))
//                .mapToDouble((e) -> e.getSalary())
		...
		mean = employees.stream().filter((e) -> e.getJobTitle().equals("부장")).mapToDouble((e) -> e.getSalary())
				.average().orElseThrow();
		System.out.println("mean=" + mean);

		// Ex6. 급여가 1,000 이상인 직원들의 정보를 한 줄에 한 명씩 출력
		System.out.println("--- Ex6. ---");
		employees.stream().filter((e) -> e.getSalary() >= 1_000).forEach(System.out::println); // e ->
																								// System.out.println(e)
		...
		mean = employees.stream().filter((e) -> e.getDept().equals("개발1팀")).mapToDouble((e) -> e.getSalary() * 1.1)
				.average().orElseThrow();
		System.out.println("mean=" + mean);
